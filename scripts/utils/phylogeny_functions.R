...
convert_edges_to_phylo <- function(tree_tbl, muts_per_cluster) {
  ...
  #
  # Args
  #   tree_tbl: tibble with two columns: From and To. Each row specifies the connection
  ...
  tip_labels = table(c(tree_tbl$From, tree_tbl$To))
  tip_labels = names(tip_labels)[tip_labels==1]
  tip_labels <- tip_labels[!str_detect(tip_labels, "PD")]
  ...
  # Define root node
  donor_id = unique(tree_tbl$From[grepl('PD', tree_tbl$From)])
  ...
  all_nodes = unique(c(tree_tbl$From, tree_tbl$To))
  ...
  # Get root
  root_lab = node_tbl[node_tbl$node_type =="root", ]$new_label
  
  # 2 --- Define edges ---
  
  # Build edge table
  edge_tbl = tree_tbl %>%
    dplyr::rename('old_child'=To, 'old_parent'=From) %>%
  ...
  ape_tbl = edge_tbl %>%
    ...
    dplyr::select(parent, child, Evidence, Assign)
  ...
  muts_per_new_cluster = left_join(
    node_tbl, muts_per_cluster %>% mutate(cluster_id = gsub("Cl.", "", cluster_id)), by=c('old_label'='cluster_id')
  ) 
  
  ape_tbl2 = ape_tbl %>%
    left_join(muts_per_new_cluster, by=c('child'='new_label')) 
  
  ape_tbl2 <- purrr::map_df(ape_tbl2, rev)
  ...
  tr <- list(edge = (as.matrix(ape_tbl2[,1:2])), 
             ...
             Nnode = num_nodes,
             edge.length = ape_tbl2$num.muts,
             node.label = filter(muts_per_new_cluster, node_type!='tip')$old_label,
             edge_strength = ape_tbl2$Evidence,
             edge_assign = ape_tbl2$Assign
  )
  class(tr) <- "phylo"
  ## unroot tree if cluster shared by all samples
  node_count <- table(unlist(ape_tbl2[,1:2]))
  if (node_count[root_lab] == 1){
    message(sprintf('All clones share single root, unrooting tree'))
     tr = unroot(tr)
   }
  checkValidPhylo(tr)
...
draw_nice_tree <- function(tr, show_internal_nodes=FALSE, edge_strength=NULL, edge_assign=NULL, patientID=id.patient,...) {
  if(is.null(edge_strength)) {
    ...
    col_tiebreak = '#F21A00'
    col_man= '#3B9AB2'
    # edge colour will indicate whether branch was automatically assigned, the result of a tiebreak (multiple possible paths), or manually assigned
    edge_col = as.character(factor(edge_assign, levels=c('auto', 'tiebreak', 'manual'), labels=c(col_auto, col_tiebreak, col_man))) 
    # edge linetype will indicate the strength of the branch assignment
    edge_lty = as.numeric(as.character(factor(edge_strength, levels=c('strong', 'medium', 'weak','very weak'), labels=c(1,6,2,3))))
    plot(tr, edge.color=edge_col, edge.lty=edge_lty, edge.width = 2,  cex = 1.75)
    legend("topright",
           legend = c('strong', 'medium', 'weak','very weak'),
           lty = c(1,6,2,3),
           lwd = 2,
           inset = c(-0.2, -0.05))
    legend("right",
           legend = c('Auto', "Tiebreak", "Manual"),
           lwd = 5,
           lty = c(1,1,1),
           inset = c(-0.2, -0.05),
           col = c("#000000", '#F21A00', '#3B9AB2'))
  }
...
draw_all_edge_sigs <- function(tr, sig_contrib_tbl, lwidth=0.2, sig_cols = NA, sig_order = NA) {
  ...
  par(xpd=TRUE, mar=c(12.1, 4.1, 4.1, 6.1))
...
draw_edge_sigs <- function(tr, edge_id, sig_contrib_tbl, lwidth=0.2, sig_cols = NA, sig_order = NA, draw_legend=FALSE) {
  ...
  sig_contrib = unlist(filter(sig_contrib_tbl, cluster_id == this_cluster)[1,which(
    grepl('SBS', names(sig_contrib_tbl)) | grepl('snv', names(sig_contrib_tbl)) )])
...
highlight_tree_nodes <- function(tr, pch=15, show_internal_nodes=FALSE, ...) {
  ...
  if(show_internal_nodes) {
    ...
    for(i in c(1:length(x))) {
      a=legend(x[i],y[i],str_replace(tr$node.label[i], 'Cl.', ''), box.col = "black", bg = "grey", adj = 0, cex=1.0, xjust=0.5, yjust=0.5, plot=FALSE)
      # box size reduced by factor 0.75
      a=a$rect
      mid = a$top - 0.5*a$h
      reduction = 0.5
      x_offset = max(x)*0.025 # shrinks width of nodes for easier visualisation
      ...
      rect(xleft=a$left+x_offset, ytop=mid+0.5*reduction*a$h, xright=a$left+(a$w-x_offset), ybottom=mid-0.5*reduction*a$h, bty='n', col = "black", border = "grey")
      text(x[i]-0.2*a$w,y[i],str_replace(tr$node.label[i], 'Cl.', ''), adj = 0, cex=1.6, xjust=0.5, yjust=0.5,col='white')
...
rename_sigs <- function(sig_name) {
  ...
  # }
  sig_name = str_replace(sig_name, 'sigP', 'PCAWG ')
