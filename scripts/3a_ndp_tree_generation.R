...
if (length(args) >= 5) {
  ...
  message("Using default snv file location of ", file.snv)
}	
...
x.cluster_and_samples <- cluster.append(snvdir =  dirname(file.snv), ndpdir = ndp_input_dir,  mut.spec.by.clust = mut.spec.by.clust, id.patient = patientID)
...
write.csv(assigned_tbl, file = paste(dirname(file.snv), paste0(patientID, "_ndp_assigned_muts.csv"), sep = "/"), quote = F, row.names = F)
...
idx = which(x.branches.keep$From %in% 'PD')
...
write.csv(muts_per_cluster, file = paste(tree_out_dir,paste(patientID, 'branch_lengths.csv',sep='.'),sep='/'), row.names = F)

### create a new ape tree object
tr = convert_edges_to_phylo(tree_tbl =  x.branches.keep.final,muts_per_cluster =   muts_per_cluster)

### save treefile
write.tree(tr, file = paste(tree_out_dir,paste(patientID, 'cluster_tree.phylo',sep='.'),sep='/'))

### plot tree
pdf(paste(tree_out_dir,paste(patientID, 'cluster_tree_rewrite.pdf',sep='.'),sep='/'))
par(xpd=TRUE, mar=c(4.1, 2.1, 2.1, 6.1), family = "sans")
draw_nice_tree(tr,show_internal_nodes=T,edge_strength=tr$edge_strength, edge_assign = tr$edge_assign, patientID=substr(patientID, 1, 7))
